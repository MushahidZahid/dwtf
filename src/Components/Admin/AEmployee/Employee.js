      ...
      <tr key={tabl.id}>
        ...
        <td>
          <Link style={{all: 'unset', cursor: 'pointer'}} to="/employee" onClick={()=>{setImgUrl('https://dwf.walnuthash.com/public/uploads/employee/' + tabl.cnic); setImgtIsOpen(true); }}>
            <img style={{width: '50px'}} src={'https://dwf.walnuthash.com/public/uploads/employee/' + tabl.cnic} alt={`Employee Cnic`}  className="img-responsive" />
          </Link>
        </td>
        <td>{tabl.phone}</td>
        <td>{tabl.salary}</td>
        ...
        <Link style={{all: 'unset', cursor: 'pointer'}} to="/employee" onClick={()=>{if (window.confirm('Are you sure you wish to delete this item?')) handleDeleteApi(tabl.id);}} > <img src={deletee} alt="" /> </Link>
           </td>
      </tr>
      );
    })}
